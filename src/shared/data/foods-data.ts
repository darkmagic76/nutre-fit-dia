import { FoodCategory } from '@domain/foodCategory';

export const foodsRaw = [
  {
    id: 'cereal-pan-integral',
    name: 'Pan integral',
    category: FoodCategory.CEREALS,
    gramsPerRation: 50,
    kcalPer100g: 240,
    proteinPer100g: 8,
    carbsPer100g: 42,
    fiberPer100g: 7,
    fatPer100g: 2,
    carbonFootprint: 0.8,
    isSeasonal: true,
  },
  {
    id: 'cereal-arroz-integral',
    name: 'Arroz integral',
    category: FoodCategory.CEREALS,
    gramsPerRation: 60,
    kcalPer100g: 350,
    proteinPer100g: 7.5,
    carbsPer100g: 73,
    fiberPer100g: 3.5,
    fatPer100g: 2.5,
    carbonFootprint: 2.7,
    isSeasonal: true,
  },
  {
    id: 'cereal-avena',
    name: 'Copos de avena integral',
    category: FoodCategory.CEREALS,
    gramsPerRation: 40,
    kcalPer100g: 370,
    proteinPer100g: 12,
    carbsPer100g: 60,
    fiberPer100g: 10,
    fatPer100g: 7,
    carbonFootprint: 0.9,
    isSeasonal: true,
  },
  {
    id: 'cereal-pasta-integral',
    name: 'Pasta integral',
    category: FoodCategory.CEREALS,
    gramsPerRation: 70,
    kcalPer100g: 340,
    proteinPer100g: 12,
    carbsPer100g: 65,
    fiberPer100g: 6,
    fatPer100g: 2,
    carbonFootprint: 1.2,
    isSeasonal: true,
  },
  {
    id: 'veg-espinaca',
    name: 'Espinaca fresca',
    category: FoodCategory.VEGETABLES,
    gramsPerRation: 150,
    kcalPer100g: 23,
    proteinPer100g: 2.9,
    carbsPer100g: 3.6,
    fiberPer100g: 2.2,
    fatPer100g: 0.4,
    carbonFootprint: 0.2,
    isSeasonal: true,
    isZeroWaste: true,
  },
  {
    id: 'veg-brocoli',
    name: 'Brócoli',
    category: FoodCategory.VEGETABLES,
    gramsPerRation: 150,
    kcalPer100g: 34,
    proteinPer100g: 2.8,
    carbsPer100g: 7,
    fiberPer100g: 2.6,
    fatPer100g: 0.4,
    carbonFootprint: 0.3,
    isSeasonal: true,
    isZeroWaste: true,
  },
  {
    id: 'veg-tomate',
    name: 'Tomate',
    category: FoodCategory.VEGETABLES,
    gramsPerRation: 150,
    kcalPer100g: 18,
    proteinPer100g: 0.9,
    carbsPer100g: 3.9,
    fiberPer100g: 1.2,
    fatPer100g: 0.2,
    carbonFootprint: 1.1,
    isSeasonal: true,
    isZeroWaste: true,
    isUglyProduce: true,
  },
  {
    id: 'veg-zanahoria',
    name: 'Zanahoria',
    category: FoodCategory.VEGETABLES,
    gramsPerRation: 100,
    kcalPer100g: 41,
    proteinPer100g: 0.9,
    carbsPer100g: 9.6,
    fiberPer100g: 2.8,
    fatPer100g: 0.2,
    carbonFootprint: 0.3,
    isSeasonal: true,
  },
  {
    id: 'veg-pimiento',
    name: 'Pimiento rojo',
    category: FoodCategory.VEGETABLES,
    gramsPerRation: 100,
    kcalPer100g: 31,
    proteinPer100g: 1,
    carbsPer100g: 6,
    fiberPer100g: 2.1,
    fatPer100g: 0.3,
    carbonFootprint: 0.5,
    isSeasonal: true,
  },
  {
    id: 'fruit-manzana',
    name: 'Manzana',
    category: FoodCategory.FRUITS,
    gramsPerRation: 150,
    kcalPer100g: 52,
    proteinPer100g: 0.3,
    carbsPer100g: 14,
    fiberPer100g: 2.4,
    fatPer100g: 0.2,
    carbonFootprint: 0.3,
    isSeasonal: true,
    isZeroWaste: true,
    isUglyProduce: true,
  },
  {
    id: 'fruit-naranja',
    name: 'Naranja',
    category: FoodCategory.FRUITS,
    gramsPerRation: 200,
    kcalPer100g: 47,
    proteinPer100g: 0.9,
    carbsPer100g: 12,
    fiberPer100g: 2.4,
    fatPer100g: 0.1,
    carbonFootprint: 0.4,
    isSeasonal: true,
  },
  {
    id: 'fruit-pera',
    name: 'Pera',
    category: FoodCategory.FRUITS,
    gramsPerRation: 150,
    kcalPer100g: 57,
    proteinPer100g: 0.4,
    carbsPer100g: 15,
    fiberPer100g: 3.1,
    fatPer100g: 0.1,
    carbonFootprint: 0.3,
    isSeasonal: true,
    isZeroWaste: true,
  },
  {
    id: 'fruit-uvas',
    name: 'Uvas',
    category: FoodCategory.FRUITS,
    gramsPerRation: 120,
    kcalPer100g: 69,
    proteinPer100g: 0.7,
    carbsPer100g: 18,
    fiberPer100g: 0.9,
    fatPer100g: 0.2,
    carbonFootprint: 0.6,
    isSeasonal: true,
  },
  {
    id: 'fruit-fresas',
    name: 'Fresas',
    category: FoodCategory.FRUITS,
    gramsPerRation: 200,
    kcalPer100g: 32,
    proteinPer100g: 0.7,
    carbsPer100g: 7.7,
    fiberPer100g: 2,
    fatPer100g: 0.3,
    carbonFootprint: 0.4,
    isSeasonal: true,
  },
  {
    id: 'oil-aove',
    name: 'Aceite de oliva virgen extra',
    category: FoodCategory.OLIVE_OIL,
    gramsPerRation: 10,
    kcalPer100g: 900,
    proteinPer100g: 0,
    carbsPer100g: 0,
    fatPer100g: 100,
    saturatedFatPer100g: 14,
    carbonFootprint: 4.5,
    isSeasonal: true,
    culturalMetadata: {
      traditionalCuisine: true,
      erMedDiet: true,
      geographicOrigin: 'Mediterráneo',
    },
  },
  {
    id: 'dairy-leche-semidesnatada',
    name: 'Leche semidesnatada',
    category: FoodCategory.DAIRY,
    gramsPerRation: 200,
    kcalPer100g: 46,
    proteinPer100g: 3.5,
    carbsPer100g: 4.8,
    fatPer100g: 1.6,
    saturatedFatPer100g: 1,
    carbonFootprint: 1.3,
    isSeasonal: true,
  },
  {
    id: 'dairy-yogur-natural',
    name: 'Yogur natural',
    category: FoodCategory.DAIRY,
    gramsPerRation: 125,
    kcalPer100g: 61,
    proteinPer100g: 4,
    carbsPer100g: 5.5,
    fatPer100g: 2.5,
    saturatedFatPer100g: 1.5,
    carbonFootprint: 1.5,
    isSeasonal: true,
  },
  {
    id: 'dairy-queso-fresco',
    name: 'Queso fresco',
    category: FoodCategory.DAIRY,
    gramsPerRation: 75,
    kcalPer100g: 160,
    proteinPer100g: 14,
    carbsPer100g: 2,
    fatPer100g: 10,
    saturatedFatPer100g: 6.5,
    carbonFootprint: 5,
    isSeasonal: true,
  },
  {
    id: 'legume-lentejas',
    name: 'Lentejas',
    category: FoodCategory.LEGUMES,
    gramsPerRation: 60,
    kcalPer100g: 340,
    proteinPer100g: 24,
    carbsPer100g: 54,
    fiberPer100g: 11,
    fatPer100g: 1.5,
    carbonFootprint: 0.8,
    isSeasonal: true,
    isZeroWaste: true,
    culturalMetadata: {
      traditionalCuisine: true,
      socialEating: true,
      cookingTechnique: 'stew' as const,
      erMedDiet: true,
    },
  },
  {
    id: 'legume-garbanzos',
    name: 'Garbanzos',
    category: FoodCategory.LEGUMES,
    gramsPerRation: 60,
    kcalPer100g: 350,
    proteinPer100g: 20,
    carbsPer100g: 55,
    fiberPer100g: 12,
    fatPer100g: 5,
    carbonFootprint: 0.8,
    isSeasonal: true,
    isZeroWaste: true,
    culturalMetadata: {
      traditionalCuisine: true,
      socialEating: true,
      cookingTechnique: 'stew' as const,
      erMedDiet: true,
    },
  },
  {
    id: 'legume-alubias',
    name: 'Alubias blancas',
    category: FoodCategory.LEGUMES,
    gramsPerRation: 60,
    kcalPer100g: 330,
    proteinPer100g: 22,
    carbsPer100g: 55,
    fiberPer100g: 15,
    fatPer100g: 1.5,
    carbonFootprint: 0.8,
    isSeasonal: true,
    culturalMetadata: {
      traditionalCuisine: true,
      socialEating: true,
      cookingTechnique: 'stew' as const,
      erMedDiet: true,
    },
  },
  {
    id: 'fish-bacalao',
    name: 'Bacalao fresco',
    category: FoodCategory.FISH,
    gramsPerRation: 150,
    kcalPer100g: 82,
    proteinPer100g: 18,
    carbsPer100g: 0,
    fatPer100g: 0.7,
    carbonFootprint: 2.2,
    isSeasonal: true,
    isHighPriority: true,
    culturalMetadata: {
      traditionalCuisine: true,
      erMedDiet: true,
      geographicOrigin: 'Atlántico Norte',
      proteinBiologicalValue: 92,
    },
  },
  {
    id: 'fish-salmon',
    name: 'Salmón',
    category: FoodCategory.FISH,
    gramsPerRation: 150,
    kcalPer100g: 208,
    proteinPer100g: 20,
    carbsPer100g: 0,
    fatPer100g: 13,
    carbonFootprint: 3.5,
    isSeasonal: true,
  },
  {
    id: 'fish-sardinas',
    name: 'Sardinas',
    category: FoodCategory.FISH,
    gramsPerRation: 150,
    kcalPer100g: 170,
    proteinPer100g: 20,
    carbsPer100g: 0,
    fatPer100g: 10,
    carbonFootprint: 1.5,
    isSeasonal: true,
    culturalMetadata: {
      traditionalCuisine: true,
      erMedDiet: true,
      geographicOrigin: 'Mediterráneo',
    },
  },
  {
    id: 'fish-merluza',
    name: 'Merluza',
    category: FoodCategory.FISH,
    gramsPerRation: 150,
    kcalPer100g: 88,
    proteinPer100g: 18,
    carbsPer100g: 0,
    fatPer100g: 1.8,
    carbonFootprint: 2.0,
    isSeasonal: true,
  },
  {
    id: 'egg-huevo',
    name: 'Huevo',
    category: FoodCategory.EGGS,
    gramsPerRation: 100,
    kcalPer100g: 150,
    proteinPer100g: 13,
    carbsPer100g: 0.5,
    fatPer100g: 10,
    saturatedFatPer100g: 3.3,
    carbonFootprint: 3.0,
    isSeasonal: true,
  },
  {
    id: 'meat-pollo',
    name: 'Pechuga de pollo',
    category: FoodCategory.WHITE_MEAT,
    gramsPerRation: 150,
    kcalPer100g: 110,
    proteinPer100g: 23,
    carbsPer100g: 0,
    fatPer100g: 1.5,
    saturatedFatPer100g: 0.5,
    carbonFootprint: 5.0,
    isSeasonal: true,
  },
  {
    id: 'meat-pavo',
    name: 'Pechuga de pavo',
    category: FoodCategory.WHITE_MEAT,
    gramsPerRation: 150,
    kcalPer100g: 105,
    proteinPer100g: 24,
    carbsPer100g: 0,
    fatPer100g: 1,
    saturatedFatPer100g: 0.3,
    carbonFootprint: 5.5,
    isSeasonal: true,
  },
  {
    id: 'meat-conejo',
    name: 'Conejo',
    category: FoodCategory.WHITE_MEAT,
    gramsPerRation: 150,
    kcalPer100g: 130,
    proteinPer100g: 22,
    carbsPer100g: 0,
    fatPer100g: 5,
    saturatedFatPer100g: 1.5,
    carbonFootprint: 4.0,
    isSeasonal: true,
  },
  {
    id: 'meat-ternera',
    name: 'Ternera',
    category: FoodCategory.RED_MEAT,
    gramsPerRation: 125,
    kcalPer100g: 175,
    proteinPer100g: 25,
    carbsPer100g: 0,
    fatPer100g: 8,
    saturatedFatPer100g: 3.5,
    carbonFootprint: 27,
    isSeasonal: true,
  },
  {
    id: 'meat-cerdo',
    name: 'Cerdo',
    category: FoodCategory.RED_MEAT,
    gramsPerRation: 125,
    kcalPer100g: 145,
    proteinPer100g: 21,
    carbsPer100g: 0,
    fatPer100g: 6.5,
    saturatedFatPer100g: 2.5,
    carbonFootprint: 7.5,
    isSeasonal: true,
  },
  {
    id: 'meat-cordero',
    name: 'Cordero',
    category: FoodCategory.RED_MEAT,
    gramsPerRation: 125,
    kcalPer100g: 210,
    proteinPer100g: 19,
    carbsPer100g: 0,
    fatPer100g: 14,
    saturatedFatPer100g: 6,
    carbonFootprint: 24,
    isSeasonal: true,
  },
  {
    id: 'water-agua',
    name: 'Agua',
    category: FoodCategory.WATER,
    gramsPerRation: 250,
    kcalPer100g: 0,
    proteinPer100g: 0,
    carbsPer100g: 0,
    fatPer100g: 0,
    carbonFootprint: 0.001,
    isSeasonal: true,
  },
  {
    id: 'proc-gazpacho-industrial',
    name: 'Gazpacho industrial',
    category: FoodCategory.VEGETABLES,
    gramsPerRation: 250,
    kcalPer100g: 45,
    proteinPer100g: 0.8,
    carbsPer100g: 4.5,
    fatPer100g: 2.5,
    harmfulIngredients: ['sacarosa'],
    isProcessed: true,
    carbonFootprint: 0.5,
  },
  {
    id: 'proc-galletas-integrales',
    name: 'Galletas integrales industriales',
    category: FoodCategory.CEREALS,
    gramsPerRation: 30,
    kcalPer100g: 430,
    proteinPer100g: 7,
    carbsPer100g: 68,
    fatPer100g: 15,
    saturatedFatPer100g: 3,
    harmfulIngredients: ['jarabe', 'maltodextrina'],
    isProcessed: true,
    carbonFootprint: 1.2,
  },
  {
    id: 'proc-zumo-industrial',
    name: 'Zumo de naranja industrial',
    category: FoodCategory.FRUITS,
    gramsPerRation: 200,
    kcalPer100g: 40,
    proteinPer100g: 0.5,
    carbsPer100g: 9,
    addedSugarsPer100g: 4,
    fatPer100g: 0.1,
    harmfulIngredients: ['concentrado de zumo', 'sirope'],
    isProcessed: true,
    carbonFootprint: 0.8,
  },
  {
    id: 'proc-refresco-cola',
    name: 'Refresco de cola',
    category: FoodCategory.WATER,
    gramsPerRation: 330,
    kcalPer100g: 42,
    proteinPer100g: 0,
    carbsPer100g: 10.6,
    addedSugarsPer100g: 10.6,
    fatPer100g: 0,
    harmfulIngredients: ['jarabe de maíz', 'sacarina'],
    hasTransFats: false,
    isProcessed: true,
  },
  {
    id: 'proc-embutido-chorizo',
    name: 'Chorizo',
    category: FoodCategory.RED_MEAT,
    gramsPerRation: 50,
    kcalPer100g: 350,
    proteinPer100g: 20,
    carbsPer100g: 1,
    fatPer100g: 30,
    saturatedFatPer100g: 11,
    harmfulIngredients: ['dextrosa', 'maltodextrina'],
    hasTransFats: true,
    isProcessed: true,
    carbonFootprint: 8,
  },
  {
    id: 'proc-margarina',
    name: 'Margarina',
    category: FoodCategory.OLIVE_OIL,
    gramsPerRation: 10,
    kcalPer100g: 720,
    proteinPer100g: 0,
    carbsPer100g: 0,
    fatPer100g: 80,
    saturatedFatPer100g: 20,
    hasTransFats: true,
    isProcessed: true,
    carbonFootprint: 3,
  },
  // ─── Nuts (AESAN 2022: frutos secos, sin sal ni azúcares añadidos) ───
  {
    id: 'nut-almendras',
    name: 'Almendras crudas',
    category: FoodCategory.NUTS,
    gramsPerRation: 25,
    kcalPer100g: 575,
    proteinPer100g: 21,
    carbsPer100g: 22,
    fiberPer100g: 12,
    fatPer100g: 49,
    saturatedFatPer100g: 4,
    carbonFootprint: 1.5,
    isSeasonal: true,
  },
  {
    id: 'nut-nueces',
    name: 'Nueces',
    category: FoodCategory.NUTS,
    gramsPerRation: 25,
    kcalPer100g: 650,
    proteinPer100g: 15,
    carbsPer100g: 14,
    fiberPer100g: 7,
    fatPer100g: 65,
    saturatedFatPer100g: 6,
    carbonFootprint: 1.2,
    isSeasonal: true,
  },
  {
    id: 'nut-avellanas',
    name: 'Avellanas',
    category: FoodCategory.NUTS,
    gramsPerRation: 25,
    kcalPer100g: 630,
    proteinPer100g: 15,
    carbsPer100g: 17,
    fiberPer100g: 10,
    fatPer100g: 61,
    saturatedFatPer100g: 5,
    carbonFootprint: 1.3,
    isSeasonal: true,
  },
  {
    id: 'nut-anacardos',
    name: 'Anacardos',
    category: FoodCategory.NUTS,
    gramsPerRation: 25,
    kcalPer100g: 550,
    proteinPer100g: 18,
    carbsPer100g: 30,
    fiberPer100g: 3,
    fatPer100g: 44,
    saturatedFatPer100g: 8,
    carbonFootprint: 2.0,
    isSeasonal: true,
  },
  {
    id: 'nut-pistachos',
    name: 'Pistachos',
    category: FoodCategory.NUTS,
    gramsPerRation: 25,
    kcalPer100g: 560,
    proteinPer100g: 20,
    carbsPer100g: 28,
    fiberPer100g: 10,
    fatPer100g: 45,
    saturatedFatPer100g: 6,
    carbonFootprint: 1.8,
    isSeasonal: true,
  },
  // ─── Tubers (AESAN 2022: tubérculos, consumo moderado) ───
  {
    id: 'tuber-patata',
    name: 'Patata',
    category: FoodCategory.TUBERS,
    gramsPerRation: 175,
    kcalPer100g: 77,
    proteinPer100g: 2.0,
    carbsPer100g: 17,
    fiberPer100g: 2.2,
    fatPer100g: 0.1,
    carbonFootprint: 0.3,
    isSeasonal: true,
  },
  {
    id: 'tuber-boniato',
    name: 'Boniato',
    category: FoodCategory.TUBERS,
    gramsPerRation: 175,
    kcalPer100g: 86,
    proteinPer100g: 1.6,
    carbsPer100g: 20,
    fiberPer100g: 3.0,
    fatPer100g: 0.1,
    carbonFootprint: 0.4,
    isSeasonal: true,
  },
  {
    id: 'tuber-name',
    name: 'Ñame',
    category: FoodCategory.TUBERS,
    gramsPerRation: 175,
    kcalPer100g: 118,
    proteinPer100g: 1.5,
    carbsPer100g: 28,
    fiberPer100g: 4.1,
    fatPer100g: 0.2,
    carbonFootprint: 0.5,
    isSeasonal: true,
  },
  // ─── Cooked legumes (preparationState: 'cooked', cooked-weight equivalents) ───
  {
    id: 'legume-lentejas-cocido',
    name: 'Lentejas cocidas',
    category: FoodCategory.LEGUMES,
    gramsPerRation: 150,
    kcalPer100g: 93,
    proteinPer100g: 6.5,
    carbsPer100g: 14,
    fiberPer100g: 5.0,
    fatPer100g: 0.5,
    carbonFootprint: 0.8,
    isSeasonal: true,
    preparationState: 'cooked' as const,
  },
  {
    id: 'legume-garbanzos-cocido',
    name: 'Garbanzos cocidos',
    category: FoodCategory.LEGUMES,
    gramsPerRation: 150,
    kcalPer100g: 120,
    proteinPer100g: 7.0,
    carbsPer100g: 17,
    fiberPer100g: 5.5,
    fatPer100g: 2.0,
    carbonFootprint: 0.8,
    isSeasonal: true,
    preparationState: 'cooked' as const,
  },
  {
    id: 'legume-alubias-cocido',
    name: 'Alubias cocidas',
    category: FoodCategory.LEGUMES,
    gramsPerRation: 150,
    kcalPer100g: 95,
    proteinPer100g: 6.0,
    carbsPer100g: 15,
    fiberPer100g: 5.5,
    fatPer100g: 0.5,
    carbonFootprint: 0.8,
    isSeasonal: true,
    preparationState: 'cooked' as const,
  },
  // ─── Cooked cereals (preparationState: 'cooked', cooked-weight equivalents) ───
  {
    id: 'cereal-arroz-integral-cocido',
    name: 'Arroz integral cocido',
    category: FoodCategory.CEREALS,
    gramsPerRation: 180,
    kcalPer100g: 123,
    proteinPer100g: 2.7,
    carbsPer100g: 26,
    fiberPer100g: 1.8,
    fatPer100g: 1.0,
    carbonFootprint: 2.7,
    isSeasonal: true,
    preparationState: 'cooked' as const,
  },
  {
    id: 'cereal-pasta-integral-cocida',
    name: 'Pasta integral cocida',
    category: FoodCategory.CEREALS,
    gramsPerRation: 180,
    kcalPer100g: 124,
    proteinPer100g: 4.5,
    carbsPer100g: 26,
    fiberPer100g: 3.5,
    fatPer100g: 0.6,
    carbonFootprint: 1.2,
    isSeasonal: true,
    preparationState: 'cooked' as const,
  },
];
