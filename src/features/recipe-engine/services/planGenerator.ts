...
/**
 * Assign specific sustainable foods to meal slots for a given day.
 */
export function assignFoodsToMeals(template: TemplateSlot[], day: number): MealEntry[] {
  return template.flatMap((slot) =>
...
/**
 * Build a single day's meal plan from the week plan context.
 * Mutates weeklyCounts accumulator in-place and returns the daily plan.
 */
export function buildDayPlan(
  day: number,
  context: WeekPlanContext,
  weeklyCounts: CountByCategory,
): DailyMeal {
  // Daily template entries
  const dailyEntries = assignFoodsToMeals(context.dailyTemplate, day);

  // Weekly-distributed slots that land on this day
  const dayWeeklySlots = context.weeklySlots.filter((s) => s.day === day);
  ...
  // Enforce AOVE in every main meal
  ...
  return { day, entries: enforcedEntries };
}
...
export function generateWeeklyPlan(restrictionActive: boolean, mealCount = 4): WeeklyPlan {
  ...
  for (let day = 1; day <= DAYS_IN_WEEK; day++) {
    days.push(buildDayPlan(day, context, weeklyCounts));
  }
