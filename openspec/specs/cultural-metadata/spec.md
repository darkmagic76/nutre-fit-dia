...
The `CulturalBadges` component **MUST** render icons conditionally: 🏺 when `traditionalCuisine` is `true`, 👥 when `socialEating` is `true`, 🌿 when `erMedDiet` is `true`. Each icon **MUST** carry a Spanish aria-label. Foods without `culturalMetadata` **MUST NOT** render badges. Badges **MUST** appear inline after the food name in `PlanView` entry rows.
...
- WHEN `PlanView` renders the entry
- THEN no badge icons appear in that entry row
