...
### P2.0 — Move planGenerator to shared/services/ (Scope Rule fix)

- [x] **P2.0.1** Move `src/features/recipe-engine/services/planGenerator.ts` → `src/shared/services/planGenerator.ts`
- [x] **P2.0.2** Move `src/features/recipe-engine/services/planGenerator.test.ts` → `src/shared/services/planGenerator.test.ts`
- [x] **P2.0.3** Move `src/features/recipe-engine/services/planGenerator.fallback.test.ts` → `src/shared/services/planGenerator.fallback.test.ts`
- [x] **P2.0.4** Create barrel re-export at `src/features/recipe-engine/services/planGenerator.ts`
- [x] **P2.0.5** Update `src/shared/stores/planStore.ts` import from `@features/recipe-engine/services/planGenerator` to `@shared/services/planGenerator`
- [x] **P2.0.6** Update `src/features/recipe-engine/RecipeEngineContainer.tsx` import
- [x] **P2.0.7** Run `pnpm typecheck` + `pnpm test:run` — all 704 tests pass
- [x] **P2.0.8** Verify: `grep -r "from.*@features" src/shared/` returns ZERO results

**Specs**: store-architecture R3-R4 (MUST NOT import from @features/)
**Effort**: S
**Deps**: P1.1 (scope rule cleanup)

### P2.1 — Add CSP + security headers

- [x] **P2.1.1** Add `server.headers` in `vite.config.ts` with dev-mode relaxed CSP (allows HMR WebSocket)
- [x] **P2.1.2** Create `public/_headers` with production CSP + `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`
- [x] **P2.1.3** Verify CSP does NOT block service worker registration, PWA manifest fetch, or Vite HMR
- [x] **P2.1.4** Run `pnpm build` — verify `dist/_headers` exists with all four headers
...
- [x] **P2.2.1** TDD RED: Write tests for `computeIMC`, `validateProfile`, `buildProfile`
- [x] **P2.2.2** GREEN: Create `src/shared/services/profileService.ts` — pure functions, zero framework imports
- [x] **P2.2.3** REFACTOR: Update `trackerStore.ts` to delegate to profileService; remove inline IMC and validation
- [x] **P2.2.4** Run `pnpm test:run` — profileService tests + trackerStore regression must pass
...
- [x] **P2.3.1** Update each View props interface: add `translate: Translations` (typed `TFunction`)
- [x] **P2.3.2** Switch View internals: `const t = useT()` → use `translate` prop for all i18n lookups
- [x] **P2.3.3** Update each Container: call `useT()`, pass `translate={t}` to its child View
- [x] **P2.3.4** Update all View test files: provide `translate={mockTranslations}` in render
- [x] **P2.3.5** Run `pnpm typecheck && pnpm test:run` — zero `useT()` imports in View files
...
- [x] **P3.1.1** RED: `src/features/activity-tracker/ActivityTrackerContainer.test.tsx`
- [x] **P3.1.2** RED: `src/features/activity-tracker/ActivityTrackerView.test.tsx`
- [x] **P3.1.3** RED: `src/features/nudge-engine/NudgeEngineContainer.test.tsx`
- [x] **P3.1.4** RED: `src/features/sustainability/SustainabilityView.test.tsx`
- [x] **P3.1.5** GREEN + REFACTOR: Write minimal impl if tests reveal bugs; prefer getByRole/getByText
...
- [x] **P3.2.1** `ErrorBoundary.test.tsx`: 9× `getByTestId` → `getByRole('alert')`, `getByText()`, `getByRole('button')`
- [x] **P3.2.2** `InstallPrompt.test.tsx`: 4× `getByTestId` → `getByRole('button')`, `getByText()`
...
- [x] **P3.3.1** `git mv DailyLogView.tsx MedDietValidatorView.tsx` + update container import + test file + barrel
- [x] **P3.3.2** `git mv ScannerView.tsx NutritionalTrafficLightView.tsx` + updates
- [x] **P3.3.3** `git mv NudgePanelView.tsx NudgeEngineView.tsx` + updates
- [x] **P3.3.4** `git mv PlanView.tsx RecipeEngineView.tsx` + updates
...
- [x] **P3.4.1** Add 5 error keys to `es.ts`: `errors.invalidGender`, `errors.diagnosisAgeExceedsCurrentAge`, `errors.glucoseRequiredForMetabolicProfile`, `errors.imcThresholdCrossedUp`, `errors.imcThresholdCrossedDown`
- [x] **P3.4.2** Add 5 error keys to `en.ts` (English translations)
- [x] **P3.4.3** Update `trackerStore.ts`: replace hardcoded Spanish strings with i18n key lookups
...
- [x] **P3.5.1** Split `buildDailyTemplate` → `buildMealSlots`, `assignFoodsToMeals`, `enforceAOVE`
- [x] **P3.5.2** Split `generateWeeklyPlan` → `initializeWeekPlan`, `buildDayPlan`
- [x] **P3.5.3** Extract `CulturalBadges.tsx` + `ZeroWasteBadges.tsx` to `recipe-engine/components/`
...
- [x] **P4.1.1** Add doc comment in `rationValidator.ts`: explains validation as ration-rule checks
- [x] **P4.1.2** Add doc comment in `errors.ts`: explains `ValidationError` as form/domain validation
- [x] **P4.1.3** Add doc comment in `DailyViolations.tsx`: explains UI-level violation display
...
- [x] **P4.2.1** Create `ModerateMinutes` value wrapper in `activity-tracker` feature
- [x] **P4.2.2** Create `GlucoseInput` value wrapper in `metabolic-tracker` feature
...
- [x] **P4.3.1** Rename `emptyCounts` → `defaultRationCounts` in `rationValidator.ts` + all references in `planGenerator.ts`
