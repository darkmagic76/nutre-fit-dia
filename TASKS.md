...
| **H3** | **Sustainability Scoring Core**   | ADR-007, FR-2.2     | ✅ **Completed** — `computeEnvironmentalScore()` with EAT-Lancet / Poore & Nemecek (2018) constants (NOT AESAN numeric data), 50/30/20 weights (internal design decision), integrated in `pickSustainableFood()` of RecipeEngine for dual ranking (health + sustainability). 14 tests. | M      | None          |
