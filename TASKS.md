...
| Offline Persistence             | ✅ Implementado (H8) — 6 stores con Zustand persist, AES-256-GCM en datos de salud, Zod Mapper.toDomain() en onRehydrateStorage, export JSON, cooldown persistido |
| HTTPS Transport (OWASP 2025)    | ✅ Implementado (RNF-04) — HTTP localhost dev, GitHub Pages HTTPS prod, CSP `upgrade-insecure-requests`                                                           |
...
| **L6** | **HTTPS Transport (OWASP)** | RNF-04       | ✅ **Completado** — HTTP localhost dev, GitHub Pages HTTPS prod. CSP `upgrade-insecure-requests`. Infraestructura pluggable sin tocar dominio.                                                                                                         | S        | Ninguna      |
| **H8** | **Offline Persistence**     | ADR-011      | ✅ **Completado** — 6 stores con Zustand persist + AES-256-GCM (datos de salud) + Zod onRehydrateStorage (Mapper.toDomain). biomarkerStore, cooldown persistido, data export JSON, locale/tab localStorage. 680 tests (66 files).                      | XL       | C5, H2, H6   |
